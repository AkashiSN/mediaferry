...
にある。2026-08-31 の回で持ち越しの 8 件が閉じ、**残っているのは次の 2 つだけ**。

- **送り先プルダウンの「変えたら 1 ページ目へ戻る」。** 一覧は 50 件で 1 ページ
  なので、**50 件を超える取り込み**をしないと 2 ページ目が無く、判定できない
**#36（同じ瞬間の日時）は 2026-08-31 に踏んだ。** リセットの「送信の記録」の段で
記録を消してから送り直すと `origin = pre_existing` になり、承認の経路に入る。
**計算は合格。見せ方の側に課題が残った**（`known-issues.md`）。

- **再確認の「差し戻しが起きる側」。** `_requeue` と stack の再オープンは、
  再計算で `captured_at` が**実際に変わったとき**にしか起動しない。値を変えるには
  プロファイルの解釈を変える必要があるが、ビルトインは編集できず、複製した
  ユーザ定義に既存のメディアは紐づかない。**実データで自然に踏むまで持ち越す**
