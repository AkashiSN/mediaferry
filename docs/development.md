...
| ~~**ジョブを積むボタンが、押したことを画面で言わない**~~（2026-08-27 に実機で発見） | **塞いだ（Phase 14）。** 時間のかかるジョブを積む操作は、積んだと言ってからホームへ送る形に揃えた。進捗と結果はホームが持つ（`/sending` は廃止）。**押したジョブは id で名指しして追う** —— 検出は実測 47 ms で終わるので、走っているものだけを出す枠には遷移した時点で何も無い。記録は [`history/phase14-record.md`](history/phase14-record.md) |
...
| **`JobStore.reap_expired_leases` がどこからも呼ばれていない**（2026-08-28 に発見） | リースが切れた `running` を倒す経路は**起動時の `sweep_interrupted` だけ**。`jobs/runner.py` の `_settle` は「決着が書けなくてもワーカーを生かす」設計なので、そこを踏んだ行は `running` のまま誰にも倒されず、**再起動するまでリセットが永久に****`job_in_flight` で断られる**（案内は「終わってから」だが、終わる主体がもう居ない）。実機で踏んだわけではない —— 2026-08-28 の断りは本物の `import` だった |
| ~~**リセットの後、ホームから取り込み直せない**~~（2026-08-28 に実機で発見） | **直して、実機で確認した（2026-08-28）。** 押した後、何もしなくても `scan` が 3 本自動で積まれ、続けて `import` まで走った（記録は [`history/hardware-verification.md`](history/hardware-verification.md)）。 「取り込んだファイル」の段は `source_entry` を消すが `scanned_at` を残していたので、ホームが `pending_count == 0` を「取り込むものは無い」と読み、**カードに元があるのに入口が消えていた**（`CardDetail` の手動スキャンからは辿れる）。`scanned_at` と、挿さっている接続の`auto_scan_at` の**両方**を落として、監視に数え直させる。**`auto_import_at` は落とさない** ——消したばかりのファイルを断りなく書き戻さない（理由は [`decisions.md`](decisions.md)） |
