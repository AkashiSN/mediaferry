...
| **`ResetNotPossible` の docstring が、断る理由を 2 つとしか書いていない**（Phase 14 の最終レビューで発見） | `db/reset.py` の `ResetNotPossible` と `reset()` の docstring は「走っている作業」「回収待ちの公開」の 2 つを挙げるが、**3 つ目（放置された claim を持つ送信レコード）が加わっている**。1 行の追記で済む |
