...
2. **チェックリストの残り。** 5 番（抜く。修正後の踏み直し）と 8 番（キャンセル）。
   **リセット後の最初の取り込みでやる**（全件が未取り込みなので、120 GiB の途中で
   好きなところで止められる）。10・11・12 番は済んだ（11 番は不合格で、対処は
   `c9cfae9`〜`5d0b842` に入れた）。
3. **`mtime` fallback の実データ確認。** 名前に時刻を持たないファイルでしか表に
   出ない。DJI では `PANORAMA/PANO_*.JPG` がそれで、**カードに 1 枚も無い**。
   パノラマを 1 枚撮って挿し直せば増分で取り込まれるので、**全件取り込みの後で
   よい**。見るのは `captured_at_source` が `mtime` で、値が現地の壁時計になること。
   **手でファイルを置く代替は勧めない** —— Linux が書く mtime は DJI の書き方と
   符号化が違いうるので、確かめたい相手を試したことにならない。
4. **A と B-2 の画面確認。** `DELETE /api/media/{id}` は実機で通した（66.31 GiB を
   回収し、行・実体とも消えて孤立を増やさなかった）が、**画面からは押していない**。
...
| **backend の応答が内部語を画面へ漏らす** | §13 の語彙表は 結合 → **つなぐ**、ジョブ → **作業の履歴** だが、**サーバが返す文字列はまだ内部語のまま** —— 送れない組の理由 `結合中` と、進捗の phase `merge` が画面に出る。そのため禁止語の一覧（`web/src/test/vocabulary.ts`）は**「結合」を入れていない**（入れると Python 側の応答文字列まで揃える必要があり、Phase 7 の範囲を越える）。画面側の文言は直したので、**残っているのはサーバ由来の語だけ**。直すなら、応答を表示用に写す層を画面に置くか、サーバの文字列そのものを §13 に合わせる |
| **§9.10 の「ゴミ箱にある」「リモートに存在しない」を画面が出していない** | `GET /uploads` は `remote_is_trashed` と `remote_asset_id` を返しているが（`api/routes_uploads.py` の `_view`）、**どの画面もこれを描いていない**。`POST /uploads/{id}/requeue`（消えた資産を送り直す）にも入口が無い。設定 › 送り先の「送り直す」が拾うのは `state=failed` だけで、ゴミ箱・消滅は `state=complete` のままなので対象外。**画面に出すには `GET /uploads` に絞り込みが要る** —— いまは `state` でしか絞れず、`complete` を 200 件読んで絞り込むと「新しい 200 件の中にたまたま無い」を「無い」と表示してしまう。`docs/user-guide.md` の該当節は**実装に合わせて「まだ画面から送り直せない」と書いてある** |
