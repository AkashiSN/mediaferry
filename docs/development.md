...
| **送り直せない理由が、`missing_at` 以外では消える**（Phase 14 の最終レビューで発見） | くわしくの「送り直す」は `presence === "gone"` と `missing_at` で出し分けるが、`check_eligibility`（`db/uploads.py`）が断る理由は他にもある（`invalidated_at` / 宛先が無効・保管済み / epoch が変わった / グループ不一致）。それらは**依然ボタンが出て 409** になり、`errors.ts` の `not_requeueable` は detail を捨てるので**理由が消える**。「押せない状態のボタンを並べない」の規則から外れている |
