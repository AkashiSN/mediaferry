...
1. **Immich への送信（Phase 3・6）。いちばん大きい未検証。** 転送先の登録、
   ...
   **一度も動かしていない**。ライブラリに実データが揃っている状態でやる方が
   安い（取り込みに 30 分かかる）。
2. **チェックリストの残り。** 10 番（取り外し）だけ。**5 番（抜く）と 8 番
   （キャンセル）は 2026-08-23 に合格**した —— 抜いた側は書きかけがその場で消え、
   回収の行もログに出た（不具合 1・2 の踏み直し）。11・12 番は済んだ（11 番は
   不合格で、対処は `c9cfae9`〜`5d0b842` に入れた）。
...
| **写真タブで、選んだ 1 件が何なのかが分からず、消せない**（2026-08-23 に実機で発見） | 4 つが 1 つの設計課題として出た。**(1) 結合物だと分からない** —— `MediaTile.tsx` が出すのは動画の尺だけで、API が返している `role: derived` を画面が使っていない。絞り込みも 6 つのうちに「できたファイル」が無い。**(2) 写真タブに削除の経路が無い** —— `Photos.tsx` にも送る画面にも `DELETE` を呼ぶところが無く、選んでできるのは送ることだけ。派生物を消せるのは 設定 › 詳しい情報 › 使っていないファイル からだけで、そこに出るのは「**もう使われていない**」出力なので、**いま現役の結合物は画面のどこからも消せない**。**(3) 1 件を選んで中身（撮影日時・尺・大きさ・どのファイルから作られたか）を見る場所が無い**。**(4)「つないだ動画の記録」が名前と中身で食い違う** —— 引いているのは `/merge-groups?status=skipped` と `/media/stale-derived` だけで、**成功した結合を 1 件も出さない**。どちらも 0 件だと画面全体が空になり、「つないだ記録」を探しに来た人には何も無いように見える。**バラバラに直すと、また片側だけになる**（B-2 の教訓）。「写真タブで、1 件を選んで、それが何かを知り、いらなければ消せる」を 1 つの設計として通す |
