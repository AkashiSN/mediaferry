...
| **ジョブを積むボタンが、押したことを画面で言わない**（2026-08-27 に実機で発見） | **つなぐボタンを押しても画面が変わらず、利用者が「失敗した」と読んだ。** `POST /merge-groups/{id}/merge` は**ジョブを積むだけ**で、`detected → merging` の書き換えは**ワーカーがジョブを拾ってから**（`db/merges.py` の `mark_merging`）。画面の `act()`（`web/src/screens/work/Merge.tsx:243`）は POST 直後に `groups.reload()` を呼ぶので、その窓に入ると **`detected` のままのカードを描き直す**（押す前と同じ見た目）。実機の実測: `created_at 23:24:27.006` → `started_at 23:24:27.471` = **465 ms の窓**。`act()` は成功を何も言わない（遷移も通知も無し）。**進捗バーはホームに出ているのに、押した人のいる場所には無い。** 不揃いは 4 か所 —— ホームの「取り込む」は留まるが同じ画面にバーが出るので気づける／カードの中身の「取り込む」とつなぐの「つなぐ」は留まるのに進捗はホーム／送るだけが `/sending` へ遷移。**この案件で 3 度目の同じ形**（Phase 7 の「`/merge` への入口が無い」、Phase 8 の「『取り外す』が成功したときに何も言わない」） |
