...
| **相方がマスクしている（構造的）** | `_beat` の `assert_lease` は、続けて呼ぶ `mark_skipped` の `_assert_current` が同じ場合を弾く | **記録に残す。** 対を同時に壊すには別ファイル（`db/uploads.py`）も要るので、単一ファイルのドライバでは当てられない。冗長さは意図であって削ってよい根拠にはならない |
