...
2. **自動取り込みが効くのは 2 度目以降の挿入から。** `_identity_confidence` は憶えた指紋が
   無ければ必ず `low` を返す（「初めて見るカードは §12.1 のとおり必ず承認を待つ」）。
   §12.1 の「一度承認すれば以後は挿すだけ」の**「以後」がここに対応する**。
   初回は承認と手動の取り込みで、2 度目の挿入から自動になる
...
- **ボタンのラベルにボリューム名を入れる。** 2 枚並ぶと「スキャン」が 2 つになり、
  ...
  検証を素通りさせる**）。
- **自動取り込みの文言は 3 つに割れる**（未承認 / 信頼済みだが確度が低い / 信頼済み）。
  未承認の文には「**次にこのカードを挿したときから**」を必ず入れる —— 承認しても
  その場では始まらない（`_identity_confidence` は憶えた指紋が無ければ必ず `low`）。
