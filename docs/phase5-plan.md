...
### 実装差分のレビュー 9 巡目（2026-08-19、codex `--fresh`。blocker 0 / major 1 / minor 2）

**同意の文言で 5 度目。** 8 巡目で条件形にしたが、条件を**時期にだけ**掛けていた。

| 指摘 | 判定 | 対処 |
| --- | --- | --- |
| **major 1**: `pending` の確認ダイアログが「いま入っている中身も含めて、以後このカードを挿すだけで NAS へコピーされます」を**先に無条件で断言**してから「確かめられない媒体では始まりません」を付け足している。`fs_uuid` の無い媒体では前半が成立せず、**同じダイアログの中で矛盾する** | **妥当。** 条件は「いつ始まるか」ではなく**許可の効果全体**に掛かる。試験も「条件句と現在の中身が両方ある」ことしか見ておらず、無条件の文を許していた | `starts` と `pending` で段落を分け、`pending` は「**確かめられた場合に限り**、いま入っている中身も含めて…コピーされます」と、条件を文全体に掛ける。`design.md` §12.1 も同じ形へ直した |
| **minor 1**: キャンセルした `recompute` に「再計算完了」イベントが残る（協調キャンセルは正常 return なので、戻り値だけを見る呼び出し側が区別できない）。ログに「中止」の後で「完了」が並ぶ | **妥当** | `RecomputeOutcome.finished` を足し、wiring がそれを見て文言を変える |
| **minor 2**: `design.md` §11 のプロファイル API 表が実装と違う（`DELETE` は無い、`/profiles/{slug}` の GET/PUT、`archive` / `duplicate` / `test` / `recompute`） | **妥当。** マージ時点の API 正本 | 表を実装に合わせ、**`DELETE` が無い理由**（使用済みリビジョンは参照が残るので消せない）も書いた |

**この巡で足した回帰試験:**

- キャンセルした実行は `finished = False` で返る
- `pending` の確認は条件を文全体に掛け、`starts` の確認だけが条件なしで約束する

**変異試験: 3 件中 3 件を検出。**

**止め時について**（レビュー側の判断）: 「指摘は明確に逓減しており、未探索の backend 層から
実害が出た状況ではない。ただし major 1 は §12.1 の同意そのものの矛盾なので、そこだけは
直してからマージするのがよい」。**その 1 件を直したのでマージへ進む。**

### 実装差分のレビュー 8 巡目（2026-08-19、codex `--fresh`。blocker 0 / major 1 / minor 2）

**同意の文言で 4 度目。** 7 巡目で `pending` を作ったが、それを**約束**として書いていた。

| 指摘 | 判定 | 対処 |
| --- | --- | --- |
| **major 1**: `identity_confidence = low` を全部 `pending` に畳んで「確かめられしだい取り込みます」と書くが、**`fs_uuid` が無い媒体と、同じ UUID の別 presence が併存している間は、何度観測しても `high` にならない**。そのカードは信頼登録しても自動では取り込まれない。API は `low` の理由を返さないので画面は区別できない | **妥当。** 7 巡目の対処は「一時的な `low`」だけを見ていた | **条件形にする**（「確かめられた場合は」）。同意の対象は変わらないので確認ダイアログはそのまま出し、そこにも「確かめられない媒体では始まりません」を添える。`design.md` §12.1 の「以後そのカードは挿すだけ」にも同じ限定を書いた。**backend から理由を返す案は採らない** —— API を増やさずに正しく書ける（増やすなら `low` の理由を安定した語彙にする設計が要る） |
| **minor 1**: `HANDOFF.md` の現在地（コミット数・総数・先頭・検証状態）がまた古い | **妥当。2 度目** | 直したうえで、**「マージの直前に実測で取り直す」と本文に書いた**（巡ごとに古くなるため） |
| **minor 2**: 7 巡目の §12.1 → §12.2 の直しが正本だけで止まり、`test_api_profiles.py` と生成物の `web/src/api/types.ts` に残っていた（typegen の比較は description を見ないので試験で捕まらない） | **妥当** | 直して `typegen` をかけ直した |

**この巡で足した回帰試験:**

- `fs_uuid` が無い媒体は、何度 `refresh()` しても `low` のまま（backend）
- 確度が低い状態を「始まる」と約束しない（画面と確認ダイアログ）

**変異試験: 2 件中 2 件を検出**（`pending` を約束形に戻す / `fs_uuid` の判定を落とす）。

**`_enqueue_ready` の排他区間には追加指摘なし**（`SettingsService.snapshot` は同一接続の
ローカル SELECT と parse だけ、`return []` も context manager が正常に COMMIT する、
入れ子のトランザクションも無い）。

### 実装差分のレビュー 7 巡目（2026-08-19、codex `--fresh`。blocker 0 / major 2 / minor 1）

**同意の文言で 3 度目。** 今度は逆向きに断言していた。

| 指摘 | 判定 | 対処 |
| --- | --- | --- |
| **major 1**: 初回観測（`identity_confidence = low`）のカードに「承認しても、いまは始まりません」と断言するが、**信頼の直後に `devices.reload()` → `GET /devices` が `refresh()` を走らせ、同じ挿入のまま `high` になって次の tick で積まれる**。「始まらない」と確認した直後に数秒で始まる | **妥当。** §3 と Task 2 の記録が「同じ挿入の次の観測で `high` になる」を確定契約にしているのに、画面がそれを否定していた | **`pending` を `blocked` から分ける**（3 状態）。`low` は「確かめられしだい始まる」、`AUTO_IMPORT=off` と `provisional` と設定未解決だけが「いまは始まりません」。**確認ダイアログは `pending` でも同意の文（いま入っている中身も含めてコピーされる）を出す** —— 変わるのは「いつ始まるか」だけ |
| **major 2**: `AUTO_IMPORT` の確認だけが enqueue の排他区間の外。読んだ後・積む前に別接続の `PUT /settings` が `off` を commit でき、**off になった後で `auto_import_at` を刻んで積める** | **妥当。** detach / archive / 信頼解除 / 版は排他の中へ移したのに、同じ「積んでよいか」の入力である設定だけ古い snapshot だった | 設定の読み取りを同じ `BEGIN IMMEDIATE` の中へ入れる |
| **minor 1**: `routes_system.py` の `DEFAULT_TIMEZONE` の参照先が §12.1 になっている（正本は §12.2） | **妥当** | 直した |

**この巡で足した回帰試験:**

- 初回観測（`low`）→ 承認 → **同じ挿入のまま** `refresh()` → `high` → 次の tick で積まれる（backend）
- 確度が低いだけなら「始まらない」と断言しない。確認ダイアログも同意の文を出す
- `AUTO_IMPORT` を読んだ後に別接続が `off` を commit できない（書き込みロックで弾かれる）

**変異試験で 1 件素通り → テストの当て方を直した。** 「ダイアログが `pending` を
`blocked` 扱いする」変異は、blocked の文にも「NAS へコピー」と理由が含まれるので
素通りした。**同意の文（「いま入っている中身も含めて」）が出ることと、
「いまは始まりません」が出ないこと**を見る形にして検出できた（4 巡連続で同じ型）。
最終的に 4 件中 4 件を検出。

**`0014` / `IN` → `=` には追加指摘なし**（未知 slug の NULL 比較、他の絞り込みとの併用、
`count(*)` の意味、他の問い合わせの実行計画をすべて確認したうえで）。
