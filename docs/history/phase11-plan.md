...
**計画の誤り（実装で分かった）: 宛先ごとの件数は出せない。** `create_pairs`
（`db/uploads.py`）は **media × destination の全組み合わせ**を作り、既に送って
ある組は `_existing` で再利用する。だから「immich-2 へ 3 件」と書くと、実際に
作られる組の数と食い違う。「選んだもの」では宛先ごとの未送信そのものが分から
ない。**確認で嘘をつくくらいなら薄いままの方がよい**ので、正確に言えるもの——
つないだ動画の内訳・撮影日の幅・打ち切り——だけを足した。
...
**計画の誤り（実装で分かった）: `FORBIDDEN` に「承認」を足さない。** 日時の確認は
**画面の言葉として**「承認する／却下する」を使っており、全画面の禁止語にすると
**その画面から操作の名前を奪う**ことになる。カードを「信頼」に寄せた時点で、
「承認」の残る場所は日時の確認 1 つに絞られる。**カードへ戻らないことは、画面
ごとの網**（`work/CardDetail.test.tsx` が本文に「承認」が無いことを見る）で足りる。
理由は `web/src/test/vocabulary.ts` に書いた。
...
- **Task 3 で、レビューに無かった崩れをもう 1 件見つけた。** 写真詳細の見出しは
  `_` しか持たないファイル名で 1 語として扱われ、390px 幅で画面の外へ出る。
  **`/photos/:id` は E2E の巡回（`SCREENS`）に入っていない**ので、Task 11 で足す
- **Task 1 と Task 2 の一部は、単体では錠を掛けられない。** jsdom が CSS を
  解析しないため。**「テストを書かない」で済ませず、Task 11 のどの検査が見るかを
  各 Task に書いた**
- **実装して分かった計画の誤りが 2 件**（Task 4 の宛先ごとの件数、Task 5 の
  禁止語）。どちらも**その Task の「危険」に追記してある** —— 消して書き直すと、
  なぜその形にしなかったかが残らない
