...
**`jobs/scan.py` に未コミットの `_sweep_vanished` がある**（カードから消えた
`source_entry` を、1 件ずつ開いて確かめてから外す）。Task 9 の前提であり、
**Phase 10 も同じファイルを大きく書き換える。** この計画に取り込むか、先に単独で
main へ入れるかを**決めてから Task 9 に入る**。放っておくと、どちらかの書き換えで
黙って消える。
...
**前提:** [先に片付けるものが 1 つある](#先に片付けるものが-1-つある)（`scan.py` の
未コミットの掃除）。**決まるまでこの Task に入らない。**
