...
### 実装差分のレビュー 1 巡目（2026-08-19、codex `--fresh`。blocker 0 / major 7 / minor 1）

`0f7d359..HEAD`（16 コミット）を見せた。**全件を反映した。退けた指摘は無い。**
blocker が無かったのは、計画レビューで backend の欠けを先に潰していたからだが、
**「実装したものが実際にはつながっていない」層**は残っていた。

| # | 指摘 | 反映 |
| --- | --- | --- |
| 1 [major] | SSE は**切断のたびに資源が残る**。Starlette は `aclose()` を呼ばずにタスクを取り消すので、`StopAsyncIteration` だけを見ていると 8 回切れば `/events` が恒久的に 503 | `_Body.__anext__` で `BaseException` を受けて返す。取り消しの経路を試験に足した |
| 2 [major] | **supersede が pending を 1 件も無効化しない。** `superseded_by_id` を立てた trigger が旧 member を `active = 0` にするので、その後に「active な member」を条件にしても当たらない | 無効化を向け直す**前**に移した（順序の罠） |
| 3 [major] | `ThumbnailCache` を要求ごとに作っており、**single-flight が効かない**（並行要求のぶんだけ ffmpeg が起動する）。起動時の掃除も無い | アプリに 1 つだけ持たせ、起動時に `evict()` する |
| 4 [major] | 一覧の取り直しが `events.length` 頼りで、**保持の上限（200 件）に達すると止まる** | 受け取った総数で判定する。初回の描画では取り直さない |
| 5 [major] | ライブラリが `POST /uploads` の**組ごとの結果を無視**し、断られた組があっても「送信を始めた」と出す | 受け付けられた宛先だけ送信を始め、断られた組と理由、開始に失敗した宛先を文に出す |
| 6 [major] | **絞り込みで隠れた選択が合計から抜ける**（30 GiB を選んで隠すと、確認は 1 MiB と出る） | 選んだものの大きさを一緒に覚える |
| 7 [major] | 手動グループ作成・regroup・宛先の編集（PATCH）・プロファイルの試行が**画面から到達できない**。E2E も結合と承認を通らない | 4 つとも画面に足し、`POST /profiles/{slug}/test` を実装（設計 §11 にあって未実装だった）。E2E は結合と承認の画面も通る |
| 8 [minor] | セッション Cookie に寿命が無く、窓を閉じるとログアウトする（DB は 14 日もつのに） | `max_age` を付けた |

**この巡で分かったこと。** 計画レビューは「**足りない**もの」を見つけ、実装差分の
レビューは「**つながっていない**もの」を見つけた（片方だけでは両方に届かない）。
7 のように「API はあるが画面から呼べない」は、E2E が通っていても素通りする ——
**受け入れの経路に入っていない機能は、無いのと同じ**。
