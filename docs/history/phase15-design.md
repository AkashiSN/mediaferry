...
#### 7 は Immich の挙動に寄りかかっている

7 の「両方 `stack: null`」は、**資産を 1 つ消したとき Immich が残り 1 件だけの
スタックも消す**ことを前提にしている。1 件のスタックが残る相手だと、JPEG の
`stack` は `null` にならず、`jobs/stacker.py` の `_adopted` が `len(stack_ids) != 1`
で `None` を返す。結果は組み直しではなく **`skipped`「相手側に別のスタックがある」**
になる。

**そうなった場合、この回の仕掛けでは回収できない。** §3.2 の新しい段が対象に
するのは `stack_state = 'stacked'` の行だが、この JPEG は `skipped` なので入らない。
利用者が Immich でその 1 件のスタックを手で消すまで、組は `skipped` のまま残る。

**この前提が崩れていたら、そのときに設計する。** 候補は「メンバーが 1 件だけの
スタックは我々のものとして引き取る」だが、`_adopted` の集合一致という
fail-closed をゆるめる話なので、実機で観測してから決める。
