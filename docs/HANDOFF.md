...
最終更新: 2026-08-18
ブランチ: `main`（単独リポジトリ、104 コミット。先頭は Phase 3 のレビュー 5 巡目の反映）
...
14 タスクを実行し、**実装差分のレビューを 3 巡（3・4・5 巡目）回して全件反映済み**。
...
**5 巡目でも blocker が 2 件出た。** 巡を重ねても止まっていない。
...
| 3 | Immich 同期（転送先プロファイル、状態機械、タグ、日時補正、複数宛先） | **完了**（`phase3-plan.md` の 14 タスク。codex レビュー 5 巡を反映。実装との差分は書き戻し済み。**実 Immich での確認だけ残り**） |
| 4 | Web UI | 未着手 |
...
```
uv run pytest                  854 passed, 4 deselected
...
| `docs/phase3-plan.md` | Phase 3（Immich 同期）の実装計画。**実行済み**。codex のレビュー 5 巡を反映し、実装で外れた判断と検出できなかった変異を書き戻してある | ✅ |
...
| **識別子は「許す形」を並べて検める（allowlist）。境界の両方向で通す** | `assetId` は `remote_asset_id` として保存され API 応答にも出る。タグの id は次の要求の URL に入る。**「正常な形」で鍵を返されると、例外文を綺麗にしただけでは平文が通る**（4 巡目の blocker）。**拒む文字を並べる検査は fail-open**（符号化した鍵 `%74%65%73%74…` は「使えない文字」を含まないのに、httpx は path に載せるし可逆に戻せる）。RFC 3986 の unreserved だけを長さの上限付きで通し、**送る値も同じ検査に通す**（検査の無かった版が保存した行が DB に残っている）。5 巡目の blocker |
...
| **保存済みの識別子も移行で落とす**（`0006`） | 受け取る側の検査は新しく受け取る値にしか効かない。旧版が保存した `remote_asset_id` は一覧の API 応答に出続け、承認と再開では次の要求の URL に入る。`complete` は無効化せず外すだけ（requeue と再確認で自力で直る）、`awaiting_datetime_approval` は指す資産が無いので無効化する。5 巡目の blocker |
| **データを作り替える移行は「既に変換済みの形」を見分ける**（`0005`） | 指紋化を入れたアプリは新しい行を指紋で保存するが、移行が無ければ `schema_migration` は前の版のまま。**生値と指紋が混ざった DB が正規に作れる。** 二重にハッシュした宛先は preflight が恒久的に拒否する。5 巡目の major |
