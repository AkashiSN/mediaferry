...
最終更新: 2026-08-20
ブランチ: **`main`**（Phase 6 はマージ済み。単独リポジトリ、計 183 コミット）
**Phase 6 は `main` に入っている。** 次は実機での確認（§1 の「残っていること」）。
**`origin/main` へは未 push**（Phase 5 のぶんまでが push 済み）。
コミット数はこのコミット時点のもので、
...
**Phase 0〜6 は実装・検証とも完了。** 実 Immich での確認も済んでいる
（2026-08-20、v3.1.0）。**Phase 6 は計画レビュー 4 巡と実装差分レビュー 4 巡
（いずれも `--fresh`）を反映し、`main` へマージ済み。**
**次は実機での確認**（§1 の「残っていること」1〜3）。新しいフェーズの予定は無い。
...
| 6 | RAW/JPEG のスタッキング、`UPLOAD_CONCURRENCY` の撤去 | **完了**（11 タスク。計画レビュー 4 巡と実装差分レビュー 4 巡を反映済み。`main` へマージ済み） |
...
**Phase 6 は `main` にマージ済み。新しいフェーズの予定は無い。**
**次は実機での確認**（§1 の「残っていること」1〜3）。いずれもこの環境では
確かめられない（TrueNAS ホスト、実 DJI、実 Canon が要る）。

**`origin/main` へは未 push。** Phase 6 のぶんを push するかは利用者の判断。
