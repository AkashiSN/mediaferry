...
最終更新: 2026-08-18
ブランチ: `main`（単独リポジトリ、105 コミット。先頭は Phase 3 のレビュー 6 巡目の反映）
...
14 タスクを実行し、**実装差分のレビューを 4 巡（3〜6 巡目）回して全件反映済み**。
...
| 5 | 4 巡目の修正差分 `bf548df..a37a0e1` | blocker 2 / major 5 / minor 1。**やはり全件が「直した箇所の周辺」。** 直した検査そのものの穴（拒否の列挙は fail-open）、直した移行が作る新しい状態（生値と指紋の混在）、直した guard の片側（後段に根拠の見直しが無い） |

| 6 | 5 巡目の修正差分 `a70e076..e164352` | blocker 2 / major 3。**直した箇所そのもの**の穴で、2 件は「直したはずの blocker が形を変えて残っていた」。原因は共通で、**秘密と比較できない場所（SQL）で値の形から素性を推定した**こと |

**この傾向は Phase 2 と同じ。** 直した箇所の周りをもう一度見せると、また出る。
**5・6 巡目とも blocker が 2 件ずつ出た。** 巡を重ねても止まっていない。
**6 巡目の教訓**: 形の推定（「64 hex なら指紋」「unreserved なら安全」）は、同じ形の
秘密を素通りさせる。素性は cohort（版）か、値自身に持たせた印（`sha256:` 接頭辞）で決める。
...
| 3 | Immich 同期（転送先プロファイル、状態機械、タグ、日時補正、複数宛先） | **完了**（`phase3-plan.md` の 14 タスク。codex レビュー 6 巡を反映。実装との差分は書き戻し済み。**実 Immich での確認だけ残り**） |
| 4 | Web UI | 未着手 |
...
```
uv run pytest                  859 passed, 4 deselected
...
| `docs/phase3-plan.md` | Phase 3（Immich 同期）の実装計画。**実行済み**。codex のレビュー 6 巡を反映し、実装で外れた判断と検出できなかった変異を書き戻してある | ✅ |
...
| **保存済みの識別子は、値を見て選り分けずに cohort ごと落とす**（`0006`） | 受け取る側の検査は新しく受け取る値にしか効かない。旧版が保存した `remote_asset_id` は一覧の API 応答に出続け、承認と再開では次の要求の URL に入る。**形で選り分けると `test-api-key` のように unreserved だけでできた鍵が残る**し、SQLite の `length`・`GLOB` は埋め込み NUL で打ち切られるのでそこもすり抜ける。**版そのものが「検査より前に書かれた行」という cohort を指せる**ので、値に関係なく外して再確認で戻す。`complete` は `remote_checked_at` も外す（残すと「リモートに存在しない」と同じ形になり、実際には在る資産を requeue で送り直せる）。5 巡目の blocker → 6 巡目の blocker |
| **変換済みかどうかは、形の推定ではなく値の接頭辞で見分ける**（`0005` / `sha256:`） | 指紋化を入れたアプリは新しい行を指紋で保存するが、移行が無ければ `schema_migration` は前の版のまま。**生値と指紋が混ざった DB が正規に作れる**ので二重ハッシュを避ける必要があるが、「64 hex なら指紋」と推定すると**同じ形の API キーを既指紋と誤認して平文を残す**（この移行が塞ぐ脅威そのもの）。指紋であることを値自身に持たせる。5 巡目の major → 6 巡目の blocker |
