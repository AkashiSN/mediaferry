...
最終更新: 2026-08-19
ブランチ: **`main`**（Phase 5 はマージ済み。単独リポジトリ、計 159 コミット）
**Phase 5 は `main` に入っている。** 次は Phase 6。
**`origin` へは push していない**（`origin/main` から 28 コミット先行。この 2 つの数は
このコミット時点のもので、**読むときは実測し直す**）。

**この種の数値は作業のたびに古くなる。** 引き継ぎ資料を更新するときは
`git rev-list --count HEAD` と検証コマンドの実測で必ず取り直すこと
（レビューで 2 度指摘された）。
...
| **自動取り込みの文言は 3 状態から作る**（`autoImportOutlook` の `starts` / `pending` / `blocked`）。**画面と確認ダイアログが同じ関数を見る** | 判定は `watcher.py` の `CANDIDATES` と同じ条件。**`pending`（確度が `high` でない）を `blocked` と混ぜない** —— 初回観測は必ず `low` だが、一覧を取り直せば同じ挿入のまま `high` になるので、「始まりません」は嘘になる。断言してよいのは `AUTO_IMPORT=off` / `provisional` / 設定未解決だけ。**確認ダイアログは `pending` でも同意の文を出す**（変わるのは「いつ始まるか」だけ）。**ただし `pending` は約束ではない** —— `fs_uuid` が無い媒体と、同じ UUID の別 presence が併存している間は何度観測しても `high` にならない。API は `low` の理由を返さないので画面は区別できず、**条件形で書く**（「確かめられた場合は」）。**レビューで 4 度出た** |
...
| **`/settings` が未解決・失敗の間は `null` のまま持ち、既定へ倒さない。信頼ボタンも押させない** | `?? "trusted"` にすると、実設定が off でも「いまの中身を数秒後にコピー」と同意を取れる。**同意の内容が作れないなら操作させない**（レビュー 4 巡目） |
| **再計算の対象は `batch_size` ごとに keyset で読む**（`rel_path` が鍵） | 全件を先に materialize すると、`media_file` 1 行ごとの相関副問い合わせが**最初の `assert_lease` より前に**リースを超えうる。`rel_path` は UNIQUE で、再計算はその列を動かさないので巡回中に行が前後しない |
...
- **major はどの巡でも 1〜3 件出続けた**（blocker は 1 件も出ていない）。1〜3 巡目は
  「直前の巡で直した箇所が作った境界」、4 巡目は 3 巡かけて誰も見ていなかった層
  （対象の抽出とリースの関係、設定が未解決のときの画面）、5 巡目はその対処が
  縛り切れていなかった次元（探索行数、解決と書き込みの間の窓）、6 巡目は
  **索引を足したら別の問い合わせの駆動索引が変わった**、7〜9 巡目は**信頼登録の
  同意文言**が 3 巡続いた（実挙動と逆 → 約束しすぎ → 条件のスコープ）
- **打ち切りはレビュー側に判断させた。** 9 巡目に「指摘は明確に逓減しており、
  未探索の backend 層から実害が出た状況ではない」と言わせてから止めた。
  依頼文に「もう指摘なしならそう言ってください」と「止め時の判断材料を一言」を
  入れると答えてくれる
- **変異試験の素通りは 4 巡連続でテストの当て方が原因だった**（手で書き写した
  問い合わせ、EXPLAIN の当て方、ダイアログの文面の見方、値だけを見た並行性の試験）。
  §8 の「素通りはまずテストの当て方を疑う」がそのまま効いた
