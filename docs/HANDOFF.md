...
最終更新: 2026-08-18
ブランチ: `main`（単独リポジトリ、107 コミット。先頭は Phase 3 のレビュー 7 巡目の反映）
...
| 7 | 6 巡目の修正差分 `e164352..c2556ec` | blocker 2 / major 1 / minor 1。**この巡から `--fresh`**（レビュー役が前巡の文脈を持たない）。継ぎ足しの 3〜6 巡目が見つけなかった穴が 2 つ出た: 適用済みの版を書き換えて**前の版の DB が開けない**、`sha256:` 接頭辞は**相手が選べる**ので出所にならない |

**この傾向は Phase 2 と同じ。** 直した箇所の周りをもう一度見せると、また出る。
**5・6・7 巡目とも blocker が 2 件ずつ出た。** 巡を重ねても止まっていない。
**7 巡目の教訓**: レビュー役を継ぎ足すと、**自分が提案した対処の周りが盲点になる**
（版の書き換えも接頭辞も、5・6 巡目のセッション自身の提案だった）。`--fresh` で回す。
...
| `docs/phase4-plan.md` | Phase 4（Web UI）の実装計画。**未着手**。17 タスク。バックエンド（認証・CSRF・SSE・サムネイル・一覧・supersede）を先に全部通してから画面へ入る | ✅ |
...
4. ~~**Phase 4（Web UI）の計画を書く。**~~ —— **書いた**（`docs/phase4-plan.md`、17 タスク）。
   **着手の前に `phase4-plan.md` の「実装の前に決めておくこと」を見る**（4 件を計画側で
   仮決めしてある: フロントの構成、認証と `BIND_HOST` の既定、セッションの保存先、
   `UPLOAD_CONCURRENCY` の効かせ方）。Phase 3 で先送りした 2 件（結合グループの破棄と
   再結合、承認待ちの差分表示）は Task 8・9 に入れてある

2 と 4 は独立して進められる。3 は TrueNAS ホストが要る。**実機で確かめ残しているのは、
ホストが要るものと実 DJI のデータが要るものだけになった。**

**Phase 4 の計画は「コードを全部は埋めない」形で書いた。** 順序と例外の流れが安全性に
効くところ（セッション・CSRF・SSE の再開位置・サムネイルのパス解決・supersede の
トランザクション）はコードで書き、画面は契約と受け入れ条件で書いてある。理由は
`phase4-plan.md` の「この計画の書き方」にある（1・2 巡目のレビューは**文書のコードの
機械的な欠陥**に費やされ、実装差分を見せた 3 巡目以降で初めて本当の層が出た）。
