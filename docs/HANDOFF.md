...
最終更新: 2026-08-19
ブランチ: `main`（単独リポジトリ、128 コミット。先頭は Phase 4 の実装差分レビューの反映）
...
（2026-08-19、v3.1.0）。**次のフェーズは 5（汎用化）。**
...
| 5 | 汎用化（Canon、プロファイル編集 UI、複数デバイス、`UPLOAD_CONCURRENCY`） | **未着手** |

### レビューで分かったこと（この案件の中核）

**codex のレビューは 9 巡回した**（Phase 3 に 7 巡、Phase 4 に 2 巡）。巡ごとの詳細は
`phase3-plan.md` と `phase4-plan.md` の「レビュー記録」にある。**傾向がはっきりしている。**
...
```
uv run pytest                  1005 passed, 4 deselected
...
**どちらも「この環境では確かめられない」もの**（1 は TrueNAS ホスト、2 は実 DJI の
データが要る）。**コードで残っている作業は無い。** 次に進めるのは Phase 5 か、
運用しながら出てくる不具合の対応になる。
...
**コードで残っている作業は無い。** 次は、この環境では確かめられない 2 件（実 USB と
実 DJI データ）か、Phase 5 のどちらか。

1. **`phase1-manual-checklist.md` を TrueNAS ホストで実行する**（§1「残っていること」1）。
   特に 11 番（mtime の解釈）は**実装の前提そのもの**なので、結果を
   `phase0-findings.md` に残す。12 番（`attached_pic`）は Phase 2 で足した項目
2. **実 DJI のデータで TS フォールバックを通す**（§1「残っていること」2）
3. **Phase 5 の計画を書く。** 範囲は `design.md` §20。中身は 4 つ:
   - `generic-dcim` / `canon-eos` のプロファイル（**Canon はカードリーダー経由の UMS のみ。
     PTP はスコープ外**と決定済み）
   - プロファイル編集 UI（Phase 4 は読み取りと `POST /profiles/{slug}/test` まで）
   - 信頼登録の UX、複数デバイスの同時接続
   - **`UPLOAD_CONCURRENCY` の多重化**（Phase 4 から意図的に送った。下記）

**計画の書き方は Phase 4 の形に倣う**（`phase4-plan.md` の「この計画の書き方」）。
順序と例外の流れが安全性に効くところだけコードで書き、画面は契約と受け入れ条件で書く。
**計画に対して 1 巡、実装差分に対して 1 巡以上、`--fresh` でレビューを回す。**

### Phase 4 から Phase 5 へ送ったもの

- **`UPLOAD_CONCURRENCY` は効かない**（送信は宛先ごとに 1 本、1 件ずつ直列）。
  多重化は**ジョブを増やす方向**で行う必要がある —— 現行の `JobRunner` は全ジョブ種で
  共通の単一 worker なので、同時実行数を上げると import / merge / scan まで並列になる。
  「宛先ごとに 1 本」を保つには claim のトランザクションで宛先単位の排他が要り、
  Phase 3 で固めたリースと停止の契約に触れる。**独立した設計課題として扱う**
- **プロファイルの編集**（読み取りと試行は Phase 4 で入った）
- **継ぎ目サムネイル**（結合の検証結果は数値と判定だけ出している）
- **動画のプレビュー再生**、**`SECRET_KEY` のローテート**
- **承認待ちの「現在値」を後から取り直す導線**（いまは承認を求めた時点の値と、その時刻を出す）

### Phase 3 で実装しなかったもの（Phase 4 で入れた / 送ったもの）

- **`UPLOAD_CONCURRENCY` は効かない。** 逐次実行に倒してあり、多重化は
  Phase 4 のワーカー多重化と一緒に入れる
- **マイグレーションはデータの作り替えが 2 本。** テーブルは
  `0004_destinations_and_uploads.sql` で足りていた。`0005` は観測値 → 指紋
  （レビュー 4 巡目）、`0006` は保存済みの識別子の掃除（5 巡目）。どちらも
  runner が接続へ渡す関数を使う（`mediaferry_fingerprint` / `mediaferry_now`。
  SQLite に SHA-256 は無く、時刻の出所は `mediaferry.clock` だけ）
- **`0005` は 5 巡目で書き換えた（版を足していない）。** 「既に指紋の値は
  変換しない」を足す修正で、**古い `0005` を適用済みの DB は runner が
  `MigrationError` で開けなくなる**（適用済みの版の書き換えは検出する設計）。
  Phase 1〜3 は配布しない前提なので、開発用 DB は作り直す
