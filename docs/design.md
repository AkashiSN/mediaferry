...
**ナビゲーションは「ホーム / 写真 / 設定」の 3 つだけ。** 広い画面ではサイドバー、狭い
画面では下部タブで、項目は同じ。エンドユーザは CLI に習熟していない前提なので、**画面の
区切りを内部の構造ではなく「利用者がしたいこと」に合わせる**。

| 場所 | 中身 |
| --- | --- |
| **ホーム** | いま挿さっているカードの帯、**やること**、進行中の作業、送り先ごとの状況、最近の取り込み |
...
動画 / 送信済み**。「まだ送っていない」は**宛先ごと**に絞れる（「宛先 D に未送信」）。
当てはまるものが 1 件も無いときは、そう書く。

**「やること」は画面が持つ一覧ではなく、状態から毎回導く。** 次の 3 つのうち、在るものだけを
この順で出す。

| やること | 条件 | 開く先 |
| --- | --- | --- |
| つなぐ | 現行の結合候補がある | **つなぐ**ページ |
| 送る | どこかの宛先に未送信のものがある | **送る**ページ |
| 確認 | `awaiting_datetime_approval` の記録がある | **確認**ページ |

**どれも無いときは「いま、やることはありません」と書く。** 空の表を出さない。

作業ページはホームの下位に置き、**ナビゲーションの項目を増やさない**。

| ページ | 内容 |
| --- | --- |
| カードの中身 | 判定結果・確度・信頼状態と、**判定の理由**（対象外のボリュームも理由付きで出す）。スキャン、取り込み、信頼登録の同意 |
| つなぐ | グループ候補の一覧。構成ファイル・ギャップ秒数・パートサイズを表示し、**なぜグループ化されたかが分かる**ようにする。閾値スライダで再計算。手動分割・結合。検証結果と継ぎ目サムネイル。失敗の再試行 |
| 確認 | `pre_existing` アセットの日時変更差分。現在値と変更案を並べて表示 |
