...
| 場所 | 中身 |
| --- | --- |
| **ホーム** | いま挿さっているカードの帯（**ラベルと容量**）、**やること**、進行中の作業、送り先ごとの状況、最近の取り込み |
...
**カードは、目の前のどれなのかが分かる形で出す。** カードの帯と「カードの中身」は
...
**「やること」は画面が持つ一覧ではなく、状態から毎回導く。** 次の 4 つのうち、在るものだけを
この順で出す。
...
| ページ | 内容 |
| --- | --- |
| カードの中身 | **ラベルと容量**、判定結果・確度・信頼状態と、**判定の理由**（対象外のボリュームも理由付きで出す）。スキャン、取り込み、信頼登録の同意 |
