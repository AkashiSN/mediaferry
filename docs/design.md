...
「最初の観測」を順序で選ぶと**同じ組が実行のたびに変わりうる**。両者の published な
観測の集合が**同じ `volume_instance` で 1 つでも重なること**を条件にする（順序に
依らない）。カード上の観測が残っていない `media_file`（再フォーマットや、同じパスに
別の中身が来て `media_file_id` が外れた場合）は、理由つきの見送りにする。
...
**ジョブのリースと、組の全員の `(target_epoch, state, remote_asset_id, invalidated_at,
stack_state)` を 1 つのトランザクションで照合する**。`_guard` と同じく
**prepare → preflight（`with_lease_pulse` で囲む）→ prepare** の 2 段にし、
**`POST` と `PUT` のそれぞれの直前で取り直す**。相手を待っている間にリースが切れる
ことも、キャンセルが commit されることもあるからである。
...
| **外部へ触る手前は、スタック専用の guard（リース + 組全員の厳密な照合）を `POST` と `PUT` の直前ごとに通す。記録は全員一致でなければ 1 行も書かない** | `complete` のレコードは claim を持たないので `prepare_side_effect` を流用できない。GET を何度か挟む間にリースは切れ、キャンセルは commit されうる。一部だけ `stacked` になると、残りが別の組として再評価される |
...
| **カード上の観測は順序で 1 つ選ばず、published な観測の集合が同じボリュームで重なることを見る** | `observed_at` は再スキャンのたびに更新されるので、「最初の観測」で選ぶと同じ組が実行のたびに変わりうる |
