...
| **上の 2 つは同じ観測で成り立つこと** | `(volume_instance_id, stem_prefix)` の組の集合が交わること | 「別のカードで stem が一致、別のカードでボリュームが一致」で通り抜ける経路 |
...
`volume_instance` に畳まれる**。したがって「複製元と複製先で、同じディレクトリの同じ
stem に、別々の撮影が入っている」場合だけは 1 つ目の条件をすり抜ける。ただしその場合も
残り 3 条件（同じ stem・**秒まで一致する `captured_at`**・同じ `captured_at_source`）を
同時に満たす必要があり、別々のシャッターでこれが成立することは実質的に無い。
**不変の観測 cohort を別に永続化する案は採らない** —— 追加する表と provenance の
維持コストが、閉じるリスクに見合わない。
