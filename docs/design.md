...
| **相手から受け取った識別子は adapter の境界で検める** | `assetId` は `remote_asset_id` として保存され API 応答にも出る。タグの id は次の要求の URL に入る。**「正常な形」で鍵を返されると、例外文を綺麗にしただけでは平文が通る。** 鍵を含む値と、経路を変えられる文字を拒む |
