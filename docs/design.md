...
`recompute_timestamps` は今も、`captured_at` を動かしたレコードの見送り（`skipped`）を
未評価へ戻す（`stacked` は戻さない）。**時刻を条件から外したので、この戻しで結果が
変わることはもう無い** —— 無駄な再評価が 1 巡するだけで害は無いため、そのままにして
ある。消すなら `recompute.py` の該当 UPDATE ごと消す（`docs/history/phase10-record.md`）。
...
**身元の判定は写真タブの一覧と同じ関数を使う**（`identity_partners`）。一覧の
`collapse=stack`（§12）は「これから組まれるはず」の予測、第 2 パスは「実際に組む」
事実だが、**別々の関数で決めると画面と結果がずれる**。身元（カード上の事実）だけを
`identity_partners` が決め、資格（`origin`・`state`・資産 ID・プロファイル）は第 2 パス
だけが見る、という分け方にする。
...
| **設定** | 自動取り込み、転送先、プロファイル編集、**詳しい情報**（作業の履歴・**つないだ後の後片付け**・使っていない派生物・接続中のカードの詳細）。env 由来の項目は錠前アイコン付きの読み取り専用 |
