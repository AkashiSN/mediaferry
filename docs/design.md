...
- **走っている作業が掴んでいるファイルは付け替えない（409）。** 送信ジョブが掴んでいる
  ...
  **承認も、書く日時は記録を掴んでから読む**（`fixing_datetime` になってから読むので、
  その間の付け替えは 409 で止まる）。走っている `recompute_timestamps` も止める理由
  になる —— 排他区間の外で値を作り、読んだ時点の上書きで書くため
...
**ホームの「まだ送っていません」は「どこにも送っていない」を指す** —— 有効な宛先が
1 つ以上あり、そのどれにも有効な記録が無いもの。宛先ごとの「未送信」は写真の一覧で
絞れる（`GET /media?status=unsent` に宛先を添える）ので、ホームの数まで宛先ごとに
すると、**片方へ送っても数が減らず、どこを見れば片付くのかが読めない**。

| 場面 | 数える／数えない |
| --- | --- |
| 有効な宛先が 1 つも無い（0 件・全部休止中・全部アーカイブ済み） | **数えない。** ホームの「やること」は**いま押せる操作**で、押す先が無い |
| 休止中・アーカイブ済みの宛先にだけ記録がある | **数える。** そこへはもう送れないので、その 1 件を頼りに「もう送ってある」とは言えない |
| 有効な宛先のどれかに記録がある（送信中・失敗・承認待ちを含む） | **数えない。** 失敗と承認待ちは、それぞれ別の枠で数えて別の操作を出している |

**条件は 1 か所に置く**（`db/selection.py` の `sent_nowhere_clause`）。`/dashboard` の
`unsent_total` と、宛先を伴わない `GET /media?status=unsent` が同じ関数を呼ぶ ——
2 か所に書くと、ホームが「N 件あります」と言いながら一覧が別の集合を出す。

**`status=unsent` だけ `destination_id` を任意にする。** 伴わなければ上の意味、
伴えばその宛先について絞る。`sent` / `failed` / `awaiting` は「どの宛先での状態か」が
決まらないと答えられないので、伴わない要求は 400 のまま。
...
「送れなかった」は送り先の話なので、送り先が 1 つも無ければ押せない。
当てはまるものが 1 件も無いときは、そう書く。

**「まだ送っていない」だけは、送り先を選ばなくても答えが決まる** —— **どの送り先にも
送っていないもの**で、ホームの「まだ送っていません」と同じ集合（§10）。送り先を選べば、
これまでどおりその送り先について絞る。**選んでいない間は、何を出しているのかを 1 行で
書く** —— チップの名前だけでは範囲が読めない。

残る 3 つ（「確認が要る」「送信済み」「送れなかった」）は、どの送り先での状態かが
決まらないと答えられないので、選ぶまで一覧を出さない（API も 400 を返す）。
...
| 送る | **どの宛先にも送っていないもの**がある（§10） | **送る**ページ |
