...
| **識別子は「許す形」を並べて検める（allowlist）。境界の両方向で通す** | `assetId` は `remote_asset_id` として保存され API 応答にも出る。タグの id は次の要求の URL に入る。**「正常な形」で鍵を返されると、例外文を綺麗にしただけでは平文が通る。** 拒む文字を並べる検査は fail-open で、`%74%65%73%74` のように符号化した鍵は「使えない文字」を 1 つも含まないのに httpx はそのまま path に載せ、可逆なので経路の先で平文へ戻る。RFC 3986 の unreserved だけを長さの上限付きで通す。**送る値も同じ検査を通す**（検査の無かった版が保存した行が DB に残っている） |
...
| **保存済みの識別子も移行で落とす**（`0006`） | 受け取る側の検査は新しく受け取る値にしか効かない。検査の無かった版が保存した `remote_asset_id` は一覧の API 応答に出続け、承認と再開では次の要求の URL に入る。形で分かるものは移行で外し（`complete` は「リモートに存在しない」と同じ形になり requeue と再確認で自力で直る。`awaiting_datetime_approval` は指す資産が無く人の操作も無意味なので無効化する）、鍵と同じ形の値は送る直前に adapter が弾く |
| **データを作り替える移行は「既に変換済みの形」を見分ける**（`0005`） | 指紋化を入れたアプリは新しい行を指紋で保存する一方、移行が無ければ `schema_migration` は前の版のままなので、**生値と指紋が混ざった DB が正規に作れる**。指紋をもう一度ハッシュすると観測値の一重指紋と永久に一致せず、その宛先が恒久的に拒否される |
